...
# @param - file name to be managed
# @param - SELinux policy file type - mandatory for "add" operation
# @param - SELinux user context
# @param - SELinux range context
...
# When uninstalling VMware Tools on a SELinux enabled system, delete any
# vmtools file context entries made during installation and configuration.
#
sub unconfigure_SELinux {
   my $is64BitUserland = is64BitUserLand();
   my $libdir = db_get_answer('LIBDIR');
   my $libsbindir = $libdir . ($is64BitUserland ? '/sbin64' : '/sbin32');
   my $libbindir = $libdir . ($is64BitUserland ? '/bin64' : '/bin32');
   ...
   if (is_selinux_enabled ()) {
      ...
      # we can only do something if the vmtools context exists
      if (direct_command('semodule -l | cut -f1 | grep vmtools | ' .
                         'grep -v permissive') ne "vmtools\n") {
         goto DONE;
      }
      if (! internal_which('semanage')) {
         print wrap("SELinux unconfigure: The 'semanage' utility was not " .
                    "found.\n");
         $error = 1;
         goto DONE;
      }
      ...
      # For those VMware Tools files that required special context on
      # a SELinux system, delete those file contexts now.
      #
      $error ||= semanageFcontext('del', db_get_answer('SBINDIR') .
                                  '/vmtoolsd');
      $error ||= semanageFcontext('del',
      ...
      if (defined(db_get_answer_if_exists('VMT_PERMISSIVE_ADDED'))) {
         if (system('semanage permissive -d vmtools_t > /dev/null 2>&1') ) {
            print wrap("SELinux unconfigure: Unable to remove vmtools_t " .
                       "permissive mapping.\n");
            $error = 1;
         }
      }
      ...
      #
      # Now remove the local SELinux contexts for any and all INITSCRPTSDIR
      # initialization scripts that were added.
      #
      foreach $service (keys %{$srv_table}){
         $initScript = db_get_answer('INITSCRIPTSDIR') . '/' .
                                     $srv_table->{$service};
         $error ||= semanageFcontext('del', $initScript);
      }
   }

DONE: {
      if($error) {
         print wrap("There was an error removing the local SELinux security " .
                    "context for VMware Tools. Please make certain SELinux " .
                    "is set up correctly.\n\n");
      }
   }
}
...
sub uninstall {
  ...
  if ($have_caf eq 'yes') {
    if (!defined($gOption{'upgrade'}) || $gOption{'upgrade'} == 0) {
      if (vmware_product() eq 'tools-for-linux') {
        if (!$open_vm_compat) {
          system('/etc/vmware-caf/pme/install/preuninstall.sh');
...
sub main {
  ...
  if (internal_basename($0) eq $cModuleUpdaterFileName) {
    ...
    if (not (($installed_version eq '10.1.10') and
