...
sub setupSELinux {
   ...
   if (is_selinux_enabled ()) {
      if (! internal_which('semodule')) {
         print wrap("SELinux setup: The 'semodule' utility was not found.\n");
         $error = 1;
         goto DONE;
      }
      ...
      if (! internal_which('semanage')) {
         print wrap("SELinux setup: The 'semanage' utility was not found.\n");
         $error = 1;
         goto DONE;
      }
      ...
      #
      $error ||= semanageFcontext('add', db_get_answer('SBINDIR') . '/vmtoolsd',
                                 'vmtools_exec_t', 'system_u');
      $error ||= semanageFcontext('add',
      ...
      if (system('semanage permissive -l | grep -q vmtools_t')) {
         if (system('semanage permissive -a vmtools_t') ) {
            print wrap('SELinux setup: Unable to set vmtools_t to ' .
                       'permissive.' . "\n");
            $error = 1;
         } else {
      ...
      foreach $service (keys %{$srv_table}){
         $initScript = db_get_answer('INITSCRIPTSDIR') . '/' .
                                     $srv_table->{$service};
         $error ||= semanageFcontext('add', $initScript, 'initrc_exec_t',
                                    'system_u');
      }
   }
...
DONE: {
      if($error) {
         print wrap("There was an error setting up the SELinux security " .
                    "context for VMware Tools. Please make sure SELinux is " .
                    "set up correctly.\n\n");
      }
   }
}
...
sub write_vmware_config {
  ...
  if (vmware_product() eq 'ws') {
    ...
    $config->set('product.version', '10.1.10');
  }
  $config->set('product.buildNumber', '6082533');
...
sub verify_solaris_packages {
  ...
  if ($pkg_cmd ne '') {
    ...
    if ($ret != 0) {
      print wrap("For full functionality of VMware Tools for Solaris, " .
                 "you need to install $pkg_name. You can use the command " .
...
sub main {
   ...
   while ($#ARGV != -1) {
	 ...
	 #        Hence this is a moot argument.
        ...
        print wrap("WARNING: clobbering kernel modules is deprecated and " .
                   "will be disabled in a future release of " .
                   vmware_product_name() . ".\n\n");
